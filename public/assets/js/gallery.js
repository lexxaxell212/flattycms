 ...
 async function loadGallery(page = 1,
  ...
  try {
   ...
   grid.innerHTML = json.data.map(p => `
    ...
    <h2 class="h5"><i class="text-primary fas fa-location-dot me-1"></i>${escHtml(p.poi_name)}</h2>
    </div>
    ...
    ${p.caption ? `<div class="text-muted small"><i class="fas fa-link me-1 text-primary"></i>${escHtml(p.caption)}</div>`: ''}
    </div>
    </div>
    </div>
    `).join('');
 ...
 async function loadReviews(page = 1, poi_id = '') {
  ...
  try {
   ...
   grid.innerHTML = json.data.map(r => `
    ...
    <img src="${r.avatar || BASE+'/assets/images/avatar.png'}" class="gal-review-card__avatar"
 ...
 if (IS_LOGGED) {
  ...
  document.getElementById('btnKirimReview').addEventListener('click',
   async () => {
    ...
    if (!poi_id) {
     flattyToast('warning', 'Lokasi belum dipilih.'); return;
    }
    if (rating < 1) {
     flattyToast('warning', 'Silahkan kasih rating ya..'); return;
    }
    if (cerita.length < 2) {
     flattyToast('warning', 'Ceritamu terlalu singkat. Min 2 karakter.'); return;
    }
  ...
  document.getElementById('btnOpenUpload')?.addEventListener('click', () => {
   flattyToast('info', 'toast.login.required');
  });
