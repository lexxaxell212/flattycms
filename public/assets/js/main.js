 ...
 async function showSuggestCard() {
  try {
   ...
   .map((item) => {
    ...
    <p class="small">${desc ? esc(trunc(desc, 90)): ""}</p>
    </div>
   ...
   console.error("Gagal ambil suggest card",
    e);
  }
 }
...
const WEATHER_TEXT = {
 ...
 "weather.refresh.label": {
  id: "Refresh cuaca", en: "Refresh weather"
 },
};
...
document.addEventListener("DOMContentLoaded", () => {
 ...
 const showFade = document.querySelectorAll(".show-fade");
 ...
 showFade.forEach((el) => observer.observe(el));
});