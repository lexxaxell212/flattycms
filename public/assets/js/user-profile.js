(function () {
 ...
 function renderPhotos() {
  ...
  grid.innerHTML = data.photos.map(p => `
   ...
   ${p.caption ? `<div class="mb-2"><i class="fas fa-thumbtack me-1"></i>${p.caption}</div>`: ''}
 ...
 function renderTrips() {
  ...
  list.innerHTML = data.trips.map(t => {
   ...
   const openBtn = isAi
   ? `<a href="/trip?open_ai=${t.id}" class="btn btn-primary btn-fit">Buka<i class="fa-solid fa-wand-magic-sparkles ms-2"></i></a>`: `<a href="/trip?open_trip=${t.id}" class="btn btn-primary btn-fit">Buka<i class="fa-solid fa-route ms-2"></i></a>`;
   return `
