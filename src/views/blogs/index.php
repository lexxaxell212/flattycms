          ...
          <section>
            <nav aria-label="breadcrumb" class="mb-1 text-muted text-decoration-none align-items-center">
                <ol class="breadcrumb small text-muted text-decoration-none align-items-center">
                    <li class="breadcrumb-item text-muted small"><a class="text-decoration-none" href="/blogs/">Blogs</a></li>
            ...
            <div class="text-muted small d-flex flex-wrap align-items-center gap-2 mb-4">
                ...
                <?php endif; ?>
                <?php if (!empty($post["cat_name"])): ?>
                    <a href="/blogs/?cat=<?= (int)($post["category_id"] ?? 0) ?>" class="badge bg-gray text-decoration-none">
                        <?= htmlspecialchars($post["cat_name"], ENT_QUOTES, "UTF-8") ?>
                    </a>
                <?php endif; ?>
            </div>
            ...
            <hr class="my-5">
            <?php require_once LIB_PATH . "v-reactions.php"; ?>
            <div class="d-flex justify-content-between align-items-center mb-4">
              <div class="justify-content-center">
                ...
                <div class="align-items-center">
                  <button id="btn-reaction" class="btn <?= $user_liked ? 'btn-primary btn-sm' : 'btn-outline-primary btn-sm' ?>" data-id="<?= $id ?>" data-type="blog">
              ...
              <div class="justify-content-center">
                ...
                <div class="gap-2 align-items-center">
                  <?php
                  $share_url   = urlencode(BASE_URL . 'blogs/?slug=' . ($post['slug'] ?? ''));
                  $share_title = urlencode($post['title'] ?? '');
                  ?>
                  <a href="https://wa.me/?text=<?= $share_title ?>%20<?= $share_url ?>" target="_blank" rel="noopener" class="btn btn-outline-primary btn-sm">
                    <i class="fa-brands fa-whatsapp"></i>
                  </a>
                  <a href="https://www.facebook.com/sharer/sharer.php?u=<?= $share_url ?>" target="_blank" rel="noopener" class="btn btn-outline-primary btn-sm">
                    <i class="fa-brands fa-facebook-f"></i>
                  </a>
                  <button onclick="copyLink()" class="btn btn-outline-primary btn-sm">
                    <i class="fa-brands fa-instagram"></i>
                  </button>
                </div>
              </div>
            </div>
            <a href="/blogs/<?= $cat_id > 0 ? "?cat=" . $cat_id : "" ?>" class="btn btn-primary">
            ...
            <article class="card card-flatty mb-4">
                <div class="card-body">
                    <a href="/blogs/?slug=<?= htmlspecialchars($p['slug'] ?? '') ?>" class="h5 mb-2">
                        <?= htmlspecialchars(mb_strlen($p['title'] ?? '') > 60 ? mb_substr($p['title'], 0, 60) . '…' : ($p['title'] ?? ''), ENT_QUOTES, 'UTF-8') ?>
                    </a>
                    <div class="d-flex align-items-center justify-content-start gap-2 small text-muted mb-3">
                        ...
                        <a href="/blogs/?cat=<?= (int)($p["category_id"] ?? 0) ?>" class="me-4 badge bg-gray text-decoration-none small">
            ...
            <nav aria-label="Navigasi halaman" class="mt-4">
                <ul class="pagination justify-content-center flex-wrap mb-4">
                    <li class="page-item <?= $page <= 1 ? "disabled" : "" ?>">
                        <a class="page-link" href="?<?= http_build_query(array_filter(["cat" => $cat_id ?: null, "page" => $page - 1])) ?>" aria-label="Halaman sebelumnya">&laquo;</a>
                    </li>
                    <?php
                    $range = 2;
                    $prev_printed = false;
                    ...
                    <li class="page-item <?= $i === $page ? "active" : "" ?>" <?= $i === $page ? 'aria-current="page"' : "" ?>>
                        <a class="page-link" href="?<?= http_build_query(array_filter(["cat" => $cat_id ?: null, "page" => $i])) ?>"><?= $i ?></a>
                    </li>
                    <?php endfor; ?>
                    <li class="page-item <?= $page >= $total_pages ? "disabled" : "" ?>">
                        <a class="page-link" href="?<?= http_build_query(array_filter(["cat" => $cat_id ?: null, "page" => $page + 1])) ?>" aria-label="Halaman berikutnya">&raquo;</a>
                    </li>
                </ul>
                <p class="text-center text-muted small mb-0">
                    Halaman <?= $page ?> dari <?= $total_pages ?> &nbsp;·&nbsp; <?= number_format($total) ?> artikel
                </p>
            </nav>
            <?php endif; ?>
        </div>
        <div class="col-md-5">
          <div style="position:sticky !important; top:82px; align-self:flex-start;">
                <div class="cat-panel">
                    ...
                    <a href="/blogs/?cat=<?= (int)($cat["id"] ?? 0) ?>" class="d-flex justify-content-between align-items-center py-2 border-bottom cat-link <?= (int)($cat["id"] ?? 0) === $cat_id ? "fw-bold" : "text-body" ?>">
                        <span><?= htmlspecialchars($cat["name"] ?? "", ENT_QUOTES, "UTF-8") ?></span>
                        <span class="badge bg-gray rounded-pill"><?= (int)($cat["post_count"] ?? 0) ?></span>
                    </a>
                    <?php endforeach; ?>
                </div>
            </div>
