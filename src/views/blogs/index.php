...
<main class="main-content">
  <div class="container">
    ...
    <div class="page-header">
      <nav aria-label="breadcrumb" class="text-muted text-decoration-none align-items-center">
      ...
      <div class="text-muted small d-flex flex-wrap align-items-center gap-1">
        ...
        <span><i class="fas fa-eye me-1"></i><?= number_format((int) $post["views"]) ?></span>
    ...
    <div class="row g-2">
      ...
      <div class="col-12">
        <img src="<?= BASE_UPLOAD_URL . htmlspecialchars($post['image'] ?? '', ENT_QUOTES, 'UTF-8') ?>" onerror="this.onerror=null;this.src='/assets/images/default.png'" alt="<?= htmlspecialchars($post['title'] ?? '', ENT_QUOTES, 'UTF-8') ?>" class="w-100 d-block mx-auto rounded-lg mb-2" style="max-width: 600px;box-shadow: var(--shadow-card);" />
        <div>
         <?= $post['copyright'] ?? '' ?>
        </div>
      </div>
          ...
          <div class="d-flex justify-content-between align-items-center mb-4">
            <div class="justify-content-center">
              ...
              <div class="align-items-center">
                <button
                  id="btn-reaction"
                  class="btn <?= $user_liked ? 'btn-primary btn-fit' : 'btn-outline-primary btn-fit' ?>" data-id="<?= $id ?>" data-type="blog">
    ...
    <div class="row g-4">
      <div class="col-md-7">
        <div class="page-header">
          ...
          <?php if (!$cat_id): ?>
          <p class="lead">
            Kumpulan artikel pilihan seputar wisata, kuliner, dan inspirasi perjalanan.
          </p>
          <?php endif; ?>
        </div>
        <?php if (empty($posts)): ?>
        ...
        <div class="card card-flatty mb-4" role="listitem">
          <div class="card-body">
            ...
            <div class="d-flex align-items-center justify-content-start gap-1 small text-muted mb-2">
              <!-- TAMBAH VIEW -->
              <?php if (!empty($p["views"]) && (int) $p["views"] > 0): ?>
              <span><i class="fas fa-eye me-1"></i><?= number_format((int) $p["views"]) ?></span>
          ...
          <div class="card-footer">
            <a href="/blogs/?slug=<?= htmlspecialchars($p['slug'] ?? '') ?>" class="btn btn-primary">
              Baca Selengkapnya
        ...
        <div style="position:sticky !important; top:calc(var(--navbar-height) + 2.5rem); align-self:flex-start;">
          <div class="cat-panel">
            ...
            <a href="/blogs/?cat=<?= (int) ($cat["id"] ?? 0) ?>"
              class="d-flex justify-content-between align-items-center mb-2 border-bottom cat-link <?= (int) ($cat["id"] ?? 0) === $cat_id ? "fw-bold" : "text-card" ?>">
              <span class="text-card"><?= htmlspecialchars($cat["name"] ?? "", ENT_QUOTES, "UTF-8") ?></span>
